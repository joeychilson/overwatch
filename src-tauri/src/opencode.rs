...
fn modern(connection: &Connection) -> Result<bool> {
...
pub fn sessions(path: &Path) -> Result<Vec<(String, i64)>> {
    let connection = open(path)?;
    let sql = if modern(&connection)? {
...
pub fn read(path: &Path, id: &str, detailed: bool) -> Result<Accumulator> {
    let connection = open(path)?;
    let modern = modern(&connection)?;
    ...
    if modern {
        let mut statement = connection.prepare("SELECT id,type,time_created,data FROM session_message WHERE session_id=?1 ORDER BY seq")?;
...
mod tests {
    ...
    fn both_database_generations_preserve_reasoning_cost_and_tool_results() -> Result<()> {
        for modern in [false, true] {
            ...
            assert_eq!(sessions(file.path())?.len(), 1);
            let result = read(file.path(), "session", true)?;
